...
public class Inventory {
    ...
    private void print(boolean isMaterial) {
        System.out.println("\n ============= Inventory =============");
    ...
    public Material openMaterial(Entity target){
        printMaterial();
        ...
        while (true){
            if (!lastDisplayedItems.isEmpty()){
                int num = input.inputInt();
                if (num == 0) return null;

                if (num > 0 && num <= lastDisplayedItems.size()){
                    return (Material) lastDisplayedItems.get(num - 1);
                } else {
                    System.out.println("Invalid selection!");
                }
            } else {
